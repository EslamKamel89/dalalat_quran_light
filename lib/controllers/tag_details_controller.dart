...
class TagDetailsController extends GetxController {
  ...
  ResponseEnum relatedTagsResponseState = ResponseEnum.initial;
  final relatedTagsEndpoint = "related-tags";
  ResponseEnum tagVideoResponseState = ResponseEnum.initial;
  ...
  Future<List<TagModel>> getRelatedTagsApi(tagId) async {
    ...
    relatedTagsResponseState = ResponseEnum.loading;
    try {
      final response = await dioConsumer.get(path);
      List data = jsonDecode(response);
      pr(data, '$t - raw response');
      if (data.isEmpty) {
        relatedTagsResponseState = ResponseEnum.success;
        pr('No relatedTagsFound found', t);
        return [];
      }
      List<TagModel> tags =
          data.map<TagModel>((json) => TagModel.fromJson(json)).toList();
      pr(tags, '$t - parsed response');
      relatedTagsResponseState = ResponseEnum.success;
      return tags;
    } on Exception catch (e) {
      pr('Exception occured: $e', t);
      relatedTagsResponseState = ResponseEnum.failed;
      // update();
      return [];
    }
  }
  ...
  Future<List<VideoModel>> getTagVideosApi(tagId) async {
    ...
    String deviceLocale = Get.locale?.languageCode ?? 'ar';
    tagVideoResponseState = ResponseEnum.loading;
    try {
      ...
      final response = await dioConsumer.get(
        path,
        queryParameter: {"tag_id": tagId},
      );
      List data = jsonDecode(response);
      pr(data, '$t - raw response');
      if (data.isEmpty) {
        tagVideoResponseState = ResponseEnum.success;
      ...
      pr('Exception occured: $e', t);
      tagVideoResponseState = ResponseEnum.failed;
      // update();
      return [];
    }
  }
