...
import 'package:dalalat_quran_light/models/RelatedArticlesModel.dart';
import 'package:dalalat_quran_light/models/RelatedTagModel.dart';
...
import 'package:dalalat_quran_light/models/video_category.dart';
...
class DataBaseHelper {
    ...
    for (var i = 0; i < linesCount; i++) {
      ...
      if (newMap[newMap.keys.first + i] != null) {
        for (var j = 0; j < newMap[newMap.keys.first + i]!.length; j++) {
          ...
          childWord.color = newMap[newMap.keys.first + i]![j]['has_tag'].toString() != 'null'
              ? Colors.red
              : Colors.black;
  ...
  Future<List<DbWordModel>> getWordByAyahId(int suraId, int ayaId) async {
    ...
    try {
      ...
      pr(queryResult, '$t raw response');
      List<DbWordModel> words = queryResult
          .map<DbWordModel>((aya) => DbWordModel.fromJsonOnlyAyaId(aya))
          .toList();
...
''';
  ...
  Future<Map<String, dynamic>> getPageByAyahNumAndSuraNum(int? ayahNumber, int? suraId) async {
    if (ayahNumber == null || suraId == null) return {};
    String query =
        '''
    SELECT
      id ,
    ...
    if (list.isEmpty) {
      pr(
        "can't find the page for ayahId: $ayahNumber"
        'getPageByAyahId',
      );
      return {};
    }
    // Map<String, dynamic> result = list[0];
    // result['sura_ar'] = await getSuraById(suraId.toString());
    // result['sura_ar'] = "hello world";
    return pr(list[0], 'getPageByAyahId');
  }
}
